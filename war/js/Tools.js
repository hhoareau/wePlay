...
function more(id, eventObj) {
	   ...
	   // On r�cup�re les donn�es sur le bloc � afficher
	   // La suite permet de choisir l'endroit o� l'on affiche (les m�thodes sont diff�rentes en fonction des navigateurs)
	   ...
	   // Nous avons d�sormais les positions d'affichages
	   if (div) {
	      // L'id envoy�e correspond bien � un bloc, on modifie son style
	      ...
	      div.style.display = 'block'; // Pass� en "block" le fait afficher
	   }
	}
...
function StringToDate(date) {
	if(date!=null){
		...
		if(date.indexOf("/")!=-1) //Si date au format text
			//TODO fonction � analyser en fonction des formats
...
function textWidth(ctx,textsize,text){
...
function getHTMLUser(u,onEnd,size){
	var rc="<div style='text-align:center;font-size:11px;margin:0px;' id='user_"+u.email+"'>";
...
function seluser(email){
    getuser(email,function(user){
       window.open("https://www.facebook.com/"+user.result.facebookid);
    });
}
...
function showPlaylist(evt){
	if(evt!=null){
		...
		var code="<table style='font-size:12px'>";
            ...
            if(playlist!=undefined) {
                playlist.forEach(function (song) {
                    ...
                    if (song.dtPlay == null) {
                        ...
                        code += "<tr><td><img src='../img/disc.png' class='small-icon'></td><td style='width:450px'>" + song.author + " - " + song.title + link + "</td><td><div id='zone_" + song.title
                        + "'></div></td><td style='width:20%'><a href='javascript:onSetScore(1,\"" + song.Id
                        + "\")'><img style='width:23px;margin:1px' src='../img/arrow_up.png'></a><a href='javascript:onSetScore(-1,\""
                        + song.Id + "\")'><img style='width:23px;margin:1px' src='../img/arrow_down.png'></a></td><td style='align-text:right,width:5%'>" + song.score + "</td></tr>";
                    }
                });
                code += "</table>";
                div.innerHTML = code;
